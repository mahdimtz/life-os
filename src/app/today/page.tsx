...
const PRIORITY_CONFIG: Record<Priority, { badge: 'danger' | 'warning' | 'success'; label: string; sort: number; filterActive: string; addActive: string }> = {
  high: { badge: 'danger', label: 'بالا', sort: 0, filterActive: 'bg-danger/15 text-danger', addActive: 'bg-danger/15 text-danger ring-1 ring-danger/30' },
  medium: { badge: 'warning', label: 'متوسط', sort: 1, filterActive: 'bg-warning/15 text-warning', addActive: 'bg-warning/15 text-warning ring-1 ring-warning/30' },
  low: { badge: 'success', label: 'پایین', sort: 2, filterActive: 'bg-success/15 text-success', addActive: 'bg-success/15 text-success ring-1 ring-success/30' },
};
...
export default function TodayPage() {
  ...
  const [addForTomorrow, setAddForTomorrow] = useState(false);
  ...
  const tomorrowTasks = useMemo(() => getTomorrowTasks(), [allStoreTasks]);
  const score = getTodayScore();
  const completedCount = allTasks.filter((t) => t.completed).length;
  const totalEstimated = allTasks.reduce((sum, t) => sum + t.estimatedMinutes, 0);

  const tasks = useMemo(() => {
    let filtered = filter === 'all' ? allTasks : allTasks.filter((t) => t.priority === filter);
    const priorityOrder = (p: Priority) => (sortDesc ? -PRIORITY_CONFIG[p].sort : PRIORITY_CONFIG[p].sort);
  ...
  }, [allTasks, filter, sortDesc]);

  const handleAdd = () => {
    const trimmed = newTitle.trim();
    if (!trimmed) return;
    const date = addForTomorrow ? getTomorrowDate() : undefined;
    addTask(trimmed, newPriority, newMinutes, undefined, newGoalId || undefined, date);
    setNewTitle('');
    setNewPriority('medium');
    setNewMinutes(30);
    setNewGoalId('');
    setShowAdd(false);
    setAddForTomorrow(false);
  };
      ...
      <div className="max-w-2xl mx-auto px-2 sm:px-4 py-6 sm:py-8 space-y-4 sm:space-y-6">
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2 text-muted">
            ...
            <span className="text-xs font-medium uppercase tracking-wider">امروز</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-bold text-text">{dateStr}</h1>
        ...
        {allTasks.length > 0 && (
          <Card className="!p-3 sm:!p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-medium text-text-secondary">پیشرفت</span>
              <span className="text-sm font-bold text-accent">{score}%</span>
            </div>
            <Progress
              value={score}
              color={score >= 80 ? 'success' : score >= 50 ? 'warning' : 'accent'}
            />
          </Card>
        )}
        ...
        {allTasks.length > 0 && (
          <div className="flex items-center justify-between gap-2 sm:gap-3">
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              <div className="flex gap-1 sm:gap-1.5">
                <button
                  ...
                  className={`px-2 sm:px-3 py-1 text-[10px] sm:text-xs rounded-lg font-medium transition-all duration-200 cursor-pointer ${
                    filter === 'all'
                ...
                {PRIORITY_OPTIONS.map((p) => (
                  <button
                    ...
                    className={`px-2 sm:px-3 py-1 text-[10px] sm:text-xs rounded-lg font-medium transition-all duration-200 cursor-pointer ${
                      filter === p
                        ? PRIORITY_CONFIG[p].filterActive
                        : 'bg-surface text-muted hover:text-text-secondary hover:bg-surface-hover'
                    }`}
                  >
            ...
            <button
              ...
              className="flex items-center gap-1 px-2 py-1 text-[10px] sm:text-xs text-muted hover:text-text-secondary bg-surface hover:bg-surface-hover rounded-lg transition-all duration-200 cursor-pointer"
            >
              <ArrowUpDown size={12} />
              {sortDesc ? 'پایین → بالا' : 'بالا → پایین'}
            </button>
          </div>
        )}
        ...
        {tasks.length > 0 ? (
          ...
          <EmptyState
            icon={Flame}
            title="ماموریتی برای امروز نیست"
            description="اولین وظیفه‌ات رو اضافه کن تا روز رو فتح کنی."
            action={
        ...
        {tomorrowTasks.length > 0 && (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-text">فردا ({tomorrowTasks.length} وظیفه)</h2>
            </div>
            <div className="space-y-2">
              {tomorrowTasks.map((task) => {
                ...
                return (
                  <Card
                    key={task.id}
                    className={`!p-3 sm:!p-4 group transition-all duration-200 border-info/20 ${
                      task.completed ? 'opacity-50' : 'hover:border-border/60'
                    }`}
                  >
                    <div className="flex items-center gap-2 sm:gap-3">
        ...
        {showAdd ? (
          <Card className="!p-3 sm:!p-4 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs sm:text-sm font-semibold text-text">ماموریت جدید</span>
              <button
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                checked={addForTomorrow}
                onChange={(e) => setAddForTomorrow(e.target.checked)}
                className="w-4 h-4 rounded border-border text-accent focus:ring-accent/30"
              />
              <span className="text-xs text-muted">اضافه کردن برای فردا</span>
            </label>
          </Card>
        ) : (
          (allTasks.length > 0 || tomorrowTasks.length > 0) && (
            <Button variant="secondary" className="w-full" onClick={() => setShowAdd(true)}>
              <Plus size={16} /> اضافه کردن ماموریت
            </Button>
          )
        )}
      </div>
